...
```shell
sudo apt install parallel detox normalize-audio mp3gain mp3info mp3check convmv detox eyed3
...
The initial approach is the most appealing. I've created 10 playlists containing 1062 MP3 songs, which will then be condensed to 983 files, totaling 66 hours of music playback. These files will consume 4.7 gigabytes of storage space. The anticipated download duration is approximately 9 minutes.
```shell
...
find ~/Downloads/CarPlaylist -type f -name "*.mp3" | nice ionice -c 3 parallel 'exiftool -Picture -b {} | convert - -resize 500x500 -quality 80 /tmp/cover_{#}.jpg && eyeD3 --remove-all-images {} && eyeD3 --add-image /tmp/cover_{#}.jpg:FRONT_COVER {} && rm /tmp/cover_{#}.jpg'
